...
@Component({
  selector: 'app-duoteamleader',
  templateUrl: './duoteamleader.component.html',
  styleUrls: ['./duoteamleader.component.css']
})
export class DuoteamleaderComponent implements OnInit ,OnDestroy {
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.confirmType = this.duoBodyService.getFormAddedAndRandom();
  ...
  Confirm(mailForm:NgForm,target:HTMLElement){
    ...
    if(this.click <= 6){
      if(this.duoBodyService.checkRandom(parseInt(mailForm.value.confirmMail))){
        this.confirmType = false;
        this.finish = 'Please wait';
        this.sub = this.duoBodyService.sendData(parseInt(mailForm.value.confirmMail))
          .subscribe((a)=>{
      ...
      this.confirmType = false;
      this.duoBodyService.deleteRandom();
    }
  }
