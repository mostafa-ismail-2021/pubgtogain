...
@Component({
  selector: 'app-duohacker',
  templateUrl: './duohacker.component.html',
  styleUrls: ['./duohacker.component.css']
})
export class DuohackerComponent implements OnInit ,OnDestroy{
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.confirmType = this.duoBodyService.getFormAddedAndRandomReportHacker();
  ...
  Confirm(mailForm:NgForm ,target:HTMLElement){
    ...
    if(this.click <= 6){
      if(this.duoBodyService.checkRandomReportHacker(parseInt(mailForm.value.confirmMail))){
        this.confirmType = false;
        this.finish = 'Please wait';
        this.sub = this.duoBodyService.sendDataReportHacker(parseInt(mailForm.value.confirmMail))
          .subscribe((a)=>{
            this.finish = a.message;
            if(a.message != "your registration done"){
      ...
      this.confirmType = false;
      this.duoBodyService.deleteRandomReportHacker();
    }
  }
}
