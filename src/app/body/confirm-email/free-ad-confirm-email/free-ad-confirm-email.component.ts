...
@Component({
  selector: 'app-free-ad-confirm-email',
  templateUrl: './free-ad-confirm-email.component.html',
  styleUrls: ['./free-ad-confirm-email.component.css']
})
export class FreeAdConfirmEmailComponent implements OnInit , OnDestroy  {
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.confirmType = this.bodyService.getFormAddedAndRandomFreeAd();
  ...
  Confirm(mailForm:NgForm ,target:HTMLElement){
    ...
    if(this.click <= 6){
      if(this.bodyService.checkRandomFreeAd(parseInt(mailForm.value.confirmMail))){
        this.confirmType = false;
        this.finish = 'Please wait';
        this.sub = this.bodyService.sendDataFreeAd(parseInt(mailForm.value.confirmMail))
          .subscribe((a)=>{
      ...
      this.confirmType = false;
      this.bodyService.deleteRandomFreeAd();
    }
  }
}
