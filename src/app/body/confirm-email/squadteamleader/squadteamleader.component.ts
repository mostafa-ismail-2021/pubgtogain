...
@Component({
  selector: 'app-squadteamleader',
  templateUrl: './squadteamleader.component.html',
  styleUrls: ['./squadteamleader.component.css']
})
export class SquadteamleaderComponent implements OnInit {
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.confirmType = this.squadBodyService.getFormAddedAndRandom();
  ...
  Confirm(mailForm:NgForm,target:HTMLElement){
    ...
    if(this.click <= 6){
      if(this.squadBodyService.checkRandom(parseInt(mailForm.value.confirmMail))){
        this.confirmType = false;
        this.finish = 'Please wait';
        this.sub = this.squadBodyService.sendData(parseInt(mailForm.value.confirmMail))
          .subscribe((a)=>{
      ...
      this.confirmType = false;
      this.squadBodyService.deleteRandom();
    }
  }
}
