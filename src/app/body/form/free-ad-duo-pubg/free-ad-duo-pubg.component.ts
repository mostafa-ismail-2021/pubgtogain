...
@Component({
  selector: 'app-free-ad-duo-pubg',
  templateUrl: './free-ad-duo-pubg.component.html',
  styleUrls: ['./free-ad-duo-pubg.component.css']
})
export class FreeAdDuoPubgComponent implements OnInit ,OnDestroy {

  
  constructor(private duoBodyService:DuobodyService , private dialog:MatDialog,private ngZone: NgZone,private dom:DomSanitizer) { }
  public YT: any;
  public player: any;
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.checkVideoIdAndRegisterType = this.duoBodyService.getVideoIdAndFreeAdType().subscribe((data:{freeAdResgister:boolean , videoId:string , videoType:string , adAppearanceCountry:string,adsWillAppear:Array<string>}) =>{
      let _this = this;
      setTimeout(function () {
  ...
  addFreeAd(personFreeAdsForm:any)
  {
    ...
    this.checkPersonFreeAd = this.duoBodyService.checkPersonFreeAd(email , youtube , this.selectedValue , this.videoId)
