...
@Component({
  selector: 'app-soloreportthehacker',
  templateUrl: './soloreportthehacker.component.html',
  styleUrls: ['./soloreportthehacker.component.css']
})
export class SoloreportthehackerComponent implements OnInit ,OnDestroy{
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.checkRegisterType = this.bodyService.getReportHackerType().subscribe((data:{reportHackerResgister : boolean,apperHackerAndWinners:boolean}) =>{
      let _this = this;
      setTimeout(function () {
  ...
  addReportHacker(personHackerForm:any)
  {
    ...
    this.checkPersonHacker = this.bodyService.checkPersonHacker(filteGmailAccont , personHackerForm.value.googleDrive)
  ...
  showPlayerState(idPubg:NgModel,target:HTMLElement){
    ...
    {
      this.showPlayerStateType =  this.bodyService.getPlayerState(idPubg.control.value).subscribe((message)=>{
