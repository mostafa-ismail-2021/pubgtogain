...
@Component({
  selector: 'app-registerduopubgmember',
  templateUrl: './registerduopubgmember.component.html',
  styleUrls: ['./registerduopubgmember.component.css']
})
export class RegisterduopubgmemberComponent implements OnInit ,OnDestroy {
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.checkVideoIdAndRegisterType = this.duoBodyService.getMemberVideoIdAndRegisterType().subscribe((data:{register:boolean , videoId:string , videoType:string , adAppearanceCountry:string,phone:boolean}) =>{
      let _this = this;
      setTimeout(function () {
  ...
  addVideo() {
    ...
    this.playercount++;
    setTimeout(function () {
      if(document.getElementById('duomemberplayer') != null || $this.playercount > 60)
      {
        try{
          $this.init();
          if(window['onYouTubeIframeAPIReady'] == undefined)
          {
            window['onYouTubeIframeAPIReady'] = (e) => {
              $this.YT = window['YT'];
              $this.reframed = false;
              $this.player = new window['YT'].Player('duomemberplayer', {
  ...
  addMemberPerson(form:any,target:HTMLElement)
  {
    if(this.click <= 6){
      ...
      this.checkMemberPerson = this.duoBodyService.getMemberEmail(
        email,
        ...
        phone
          ...
          else if(data.checkExistsBefore)
          {
            ...
            this.duoBodyService.confirmMemberEmail(
              email ,
              form.form.value.Idpubg,
              form.form.value.teamLeaderIdpubg,
              phone,
