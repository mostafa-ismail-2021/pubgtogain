...
export class DuobodyService {
  ...
  checkPerson(
    ...
    videoId:string){
      return new Observable(observer => {
        this.http.post<{existEmail:boolean , existIdPubg:boolean ,memberBlackList:boolean , existPhone:boolean , random:number}>(BACKEND_URL + 'checkPerson',{email:email , pubgId :pubgId , pubgIdMember:memberIdpubg , playerPhone:phone})
        .subscribe((data:{existEmail:boolean , existIdPubg:boolean ,memberBlackList:boolean , existPhone:boolean , random:number})=>{
          if(data.random == null){
            observer.next({existEmail:data.existEmail , existIdPubg:data.existIdPubg , memberBlackList:data.memberBlackList , existPhone:data.existPhone});
            observer.complete();
          }
          else{
            this.formAdded = {
  ...
  getMemberEmail(
    ...
    phone:string){
      return new Observable(observer => {
        this.http.post<{random:number,checkEmail:boolean ,checkIdPubg:boolean , checkTeamleaderIdPubg:boolean, checkExistsBefore:boolean , checkPhone:boolean , error:boolean}>(BACKEND_URL + 'sendMemberEmailConfirm',{Email : Email, pubgId : pubgId , teamLeaderIdpubg : teamLeaderIdpubg , playerPhone:phone})
        .subscribe(data=>{
