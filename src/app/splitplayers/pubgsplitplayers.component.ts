...
@Component({
  selector: 'app-pubgsplitplayers',
  templateUrl: './pubgsplitplayers.component.html',
  styleUrls: ['./pubgsplitplayers.component.css']
})
export class PubgsplitplayersComponent implements OnInit , OnDestroy{
  ...
  ngOnInit(): void {
    window.scrollTo(0, 0);
    this.getVideoIdAndRegisterType = this.bodyService.getVideoIdAndDateType().subscribe((data:{dateAppearance:boolean , videoId:string , videoType:string , adAppearanceCountry:string}) =>{
      let _this = this;
      setTimeout(function () {
  ...
  addVideo() {
    ...
    this.playercount++;
    setTimeout(function () {
      if(document.getElementById('solodateplayer') != null || $this.playercount > 60)
      {
        try{
          $this.init();
          if(window['onYouTubeIframeAPIReady'] == undefined)
          {
            window['onYouTubeIframeAPIReady'] = (e) => {
              $this.YT = window['YT'];
              $this.reframed = false;
              $this.player = new window['YT'].Player('solodateplayer', {
  ...
  showDate(idPubg:NgModel,target:HTMLElement){
    ...
    {
      this.showDateType =  this.bodyService.getPlayerDate(idPubg.control.value).subscribe((data:{date:string , time:number,message:string})=>{
